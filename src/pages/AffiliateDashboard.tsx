      ...
      <div className="py-8 space-y-12">
        ...
        {(!isPlatformActive && loading) ? (
          ...
          <motion.div 
            ...
            className="card-luxury p-4 sm:p-10 bg-white border border-[#D8E3FF] text-text-1 space-y-6 sm:space-y-10 relative overflow-hidden shadow-xs rounded-3xl"
          >
             <div className="space-y-2 relative z-10">
                <p className="text-text-3 text-[10px] font-black uppercase tracking-[0.3em] leading-none opacity-80">{t('affiliate.endowment')}</p>
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
                  <p className="text-5xl sm:text-6xl font-serif font-black tracking-tight text-text-1">
                    <span className="text-[#2563EB] text-3xl sm:text-4xl mr-2">{userCurrency === 'USD' ? '$' : '₦'}</span>{calculatedBalance.toLocaleString()}
                  </p>
                  <button 
                    onClick={handleWithdrawalClick}
                    disabled={loading || calculatedBalance < (userCurrency === 'USD' ? 10 : 10000)}
                    className="w-full sm:w-auto justify-center bg-[#2563EB] text-white px-8 py-4 sm:py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-md shadow-blue-500/20 hover:bg-blue-600 active:scale-95 transition-all disabled:opacity-30 disabled:grayscale flex items-center gap-2 cursor-pointer"
                  >
             ...
             <div className="bg-[#EEF3FF] p-4 sm:p-8 rounded-2xl border border-[#D8E3FF] space-y-6 relative z-10">
                <div className="flex items-center justify-between px-2">
                   <div className="flex items-center gap-3">
                      <Landmark className={cn("w-5 h-5", profile?.bankDetails?.accountNumber ? "text-[#2563EB]" : "text-text-3 opacity-30")} />
                      <span className="text-[10px] font-black uppercase tracking-[0.4em] text-text-3">{t('affiliate.payout_creds')}</span>
                   </div>
                   <button 
                    onClick={() => setShowBankForm(true)}
                    className="text-[10px] font-black text-[#2563EB] uppercase tracking-widest hover:underline flex items-center gap-2 cursor-pointer"
                   >
                ...
                {profile?.bankDetails?.accountNumber ? (
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-2">
                    <div className="space-y-1">
                      ...
                      <p className="text-[11px] font-serif font-black text-text-1">{profile.bankDetails.bankName}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[8px] text-text-3 font-black uppercase tracking-widest leading-none">{t('affiliate.access_id')}</p>
                      <p className="text-[11px] font-mono font-black text-[#2563EB] tracking-widest">{profile.bankDetails.accountNumber}</p>
                    </div>
                  </div>
                ) : (
             ...
             <div className="bg-[#EEF3FF] p-4 sm:p-8 rounded-2xl border border-[#D8E3FF] space-y-6 relative z-10">
                <div className="flex items-center justify-between px-2">
                   ...
                   <span className="bg-white px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-[#2563EB] border border-[#D8E3FF] shadow-xs">{profile?.referralCode || fallbackCode}</span>
                </div>
               <div className="relative group">
                  <input 
                    readOnly 
                    value={referralLink}
                    className="w-full bg-white border border-[#D8E3FF] px-4 sm:px-6 py-4 rounded-2xl text-[11px] font-mono pr-16 focus:outline-none truncate text-[#2563EB] transition-all"
                  />
                  <button 
                    onClick={copyToClipboard}
                    className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-[#2563EB] text-white rounded-xl flex items-center justify-center hover:bg-blue-600 active:scale-95 transition-all shadow-xs cursor-pointer"
                  >
                    <Copy className="w-4 h-4" />
                  </button>
               </div>
               {copied && (
                  <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[10px] text-[#2563EB] font-black text-center uppercase tracking-widest animate-pulse">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
           <AffStat icon={Users} label={t('affiliate.referred_assets')} value={referrals.length} color="text-[#2563EB]" bg="bg-[#EEF3FF]" />
           <AffStat icon={Diamond} label={t('affiliate.total_earned')} value={`${userCurrency === 'USD' ? '$' : '₦'}${totalEarned.toLocaleString()}`} color="text-[#2563EB]" bg="bg-[#EEF3FF]" />
           <AffStat icon={Zap} label={t('affiliate.rev_share')} value="25%" color="text-emerald-600" bg="bg-emerald-50" />
           <AffStat icon={Wallet} label={t('affiliate.net_balance')} value={`${userCurrency === 'USD' ? '$' : '₦'}${calculatedBalance.toLocaleString()}`} color="text-[#2563EB]" bg="bg-[#EEF3FF]" />
        </div>
        ...
        <section className="space-y-8">
          ...
          <div className="grid gap-4">
            {commissions.length > 0 ? commissions.map((c) => (
              <div key={c.id} className="card-luxury p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-[#D8E3FF] bg-white rounded-2xl shadow-xs hover:border-[#2563EB] transition-all group">
                 <div className="flex items-center space-x-4 sm:space-x-5">
                    <div className="w-12 h-12 bg-[#EEF3FF] rounded-2xl flex items-center justify-center text-[#2563EB] border border-[#D8E3FF] shrink-0">
                 ...
                 <div className="text-left sm:text-right w-full sm:w-auto pl-16 sm:pl-0">
                    ...
                    <p className="text-[8px] text-text-3 uppercase font-black tracking-widest opacity-60">25% {t('affiliate.rev_share')}</p>
                 </div>
              </div>
            )) : (
              <div className="text-center py-12 card-luxury border border-dashed border-[#D8E3FF] bg-white rounded-2xl">
                <Clock className="w-12 h-12 text-blue-200 mx-auto mb-4" />
                <p className="text-[10px] font-black text-text-3 uppercase tracking-[0.4em] italic opacity-60">{t('affiliate.awaiting_endowment')}</p>
              </div>
            )}
          </div>
        </section>
        ...
        <section className="space-y-8">
          ...
          <div className="grid gap-4">
            {withdrawals.length > 0 ? withdrawals.map((w) => (
              <div key={w.id} className="card-luxury p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-[#D8E3FF] bg-white rounded-2xl shadow-xs">
                 <div className="flex items-center space-x-4 sm:space-x-5">
                    <div className="w-12 h-12 bg-[#EEF3FF] rounded-2xl flex items-center justify-center text-[#2563EB] border border-[#D8E3FF] shrink-0">
                 ...
                 <div className="pl-18 sm:pl-0 w-full sm:w-auto flex justify-start sm:justify-end">
                    <div className={cn(
                       ...
                       w.status === 'success' ? "text-emerald-600 bg-emerald-50 border-emerald-200" :
                       w.status === 'failed' ? "text-red-600 bg-red-50 border-red-200" :
                       "text-[#2563EB] bg-[#EEF3FF] border-[#D8E3FF]"
                    )}>
                       {w.status}
                    </div>
                 </div>
              ...
              <div className="text-center py-12 card-luxury border border-dashed border-[#D8E3FF] bg-white rounded-2xl">
                <Clock className="w-12 h-12 text-blue-200 mx-auto mb-4" />
                <p className="text-[10px] font-black text-text-3 uppercase tracking-[0.4em] italic opacity-60">{t('affiliate.no_redistributed')}</p>
              </div>
            )}
          </div>
        </section>
        ...
        {showBankForm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div 
              ...
              className="w-full max-w-md bg-white border border-[#D8E3FF] rounded-3xl p-8 shadow-xl relative z-10 space-y-6"
            >
              <div className="text-center space-y-2">
                <div className="w-14 h-14 bg-[#EEF3FF] rounded-2xl flex items-center justify-center text-[#2563EB] mx-auto border border-[#D8E3FF] mb-2">
                  <Landmark className="w-7 h-7" />
                </div>
                <h3 className="text-xl font-serif font-black text-text-1 tracking-tight">{t('affiliate.payout_authority')}</h3>
              ...
              <form onSubmit={saveBankDetails} className="space-y-4">
                  ...
                  <>
                    <div className="space-y-1.5">
                      ...
                      <select 
                        ...
                        className="w-full bg-[#EEF3FF] border border-[#D8E3FF] rounded-2xl px-5 py-3.5 text-[13px] text-text-1 focus:border-[#2563EB] outline-none transition-all"
                      >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input 
                        ...
                        className="w-full bg-[#EEF3FF] border border-[#D8E3FF] rounded-2xl px-5 py-3.5 text-[13px] text-text-1 focus:border-[#2563EB] outline-none transition-all uppercase font-bold"
                      />
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input 
                        ...
                        value={bankDetails.accountNumber}
                        onChange={(e) => setBankDetails({...bankDetails, accountNumber: e.target.value})}
                        className="w-full bg-[#EEF3FF] border border-[#D8E3FF] rounded-2xl px-5 py-3.5 text-[13px] text-text-1 focus:border-[#2563EB] outline-none transition-all font-mono tracking-widest"
                      />
                    </div>
                ...
                <button 
                  type="submit"
                  disabled={loading}
                  className="w-full h-14 bg-[#2563EB] text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-md shadow-blue-500/20 hover:bg-blue-600 active:scale-95 transition-all flex items-center justify-center gap-3 cursor-pointer"
                >
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <div className="text-center space-y-2">
                ...
                <h3 className="text-xl font-serif font-black text-text-1 tracking-tight">
                ...
                <p className="text-[10px] font-black text-text-3 uppercase tracking-widest">
                  {language === 'fr' ? 'Solde disponible :' : 'Available Balance:'} <span className="text-[#2563EB]">{userCurrency === 'USD' ? '$' : '₦'}{calculatedBalance.toLocaleString()}</span>
                </p>
              </div>
                ...
                <form onSubmit={submitWithdrawalRequest} className="space-y-6">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <div className="absolute left-5 top-1/2 -translate-y-1/2 text-[#2563EB] font-serif font-black text-lg">
                        {userCurrency === 'USD' ? '$' : '₦'}
                      </div>
                      <input 
                        type="number"
                        ...
                        className="w-full bg-[#EEF3FF] border border-[#D8E3FF] rounded-2xl pl-10 pr-20 py-3.5 text-[15px] font-black text-text-1 focus:border-[#2563EB] outline-none transition-all font-mono tracking-widest"
                      />
                      <button
                        type="button"
                        onClick={() => setWithdrawAmount(calculatedBalance.toString())}
                        className="absolute right-3 top-1/2 -translate-y-1/2 bg-[#2563EB] text-white hover:bg-blue-600 px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-wider transition-all cursor-pointer"
                      >
                        MAX
                      </button>
                    </div>
                    <div className="flex justify-between items-center px-1">
                      <p className="text-[9px] text-text-3 uppercase tracking-widest font-black opacity-60">
                        {language === 'fr' ? 'Minimum requis :' : 'Minimum Required:'} <span className="text-[#2563EB]">{userCurrency === 'USD' ? '$10' : '₦10,000'}</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    <button 
                      type="button"
                      onClick={() => setShowWithdrawModal(false)}
                      className="w-1/2 h-14 bg-white border border-[#D8E3FF] text-text-2 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:border-[#2563EB] transition-all cursor-pointer"
                    >
...
function AffStat({ icon: Icon, label, value, color, bg }: any) {
  return (
    <div className="card-luxury p-4 sm:p-6 space-y-3 sm:space-y-4 bg-white border border-[#D8E3FF] rounded-3xl shadow-xs flex flex-col justify-between group hover:border-[#2563EB] transition-all">
      <div className={cn("p-3 rounded-2xl w-fit border border-[#D8E3FF]", bg)}>
         <Icon className={cn("w-5 h-5", color)} />
      </div>
      <div className="space-y-1">
         <p className="text-2xl font-serif font-black text-text-1 tracking-tight group-hover:text-[#2563EB] transition-colors">{value}</p>
